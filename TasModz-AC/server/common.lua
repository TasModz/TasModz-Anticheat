...
RegisterServerEvent('tasifyanticheat:triggerServerCallback')
AddEventHandler('tasifyanticheat:triggerServerCallback', function(name, requestId, token, ...)
    local _source = source

    if (TAC.ValidateOrKick(_source, GetCurrentResourceName(), token)) then
        TAC.TriggerServerCallback(name, _source, function(...)
            TriggerClientEvent('tasifyanticheat:serverCallback', _source, requestId, ...)
        end, ...)
    end
end)

RegisterServerEvent('tasifyanticheat:triggerServerEvent')
AddEventHandler('tasifyanticheat:triggerServerEvent', function(name, token, ...)
    local _source = source
