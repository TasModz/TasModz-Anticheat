...
TAC.LoadConfig = function()
    ...
    TAC.Config = {
        UpdateIdentifiers   = TAC.GetConfigVariable('tasifyanticheat.updateidentifiers', 'boolean'),
        GodMode             = TAC.GetConfigVariable('tasifyanticheat.godmode', 'boolean'),
        Webhook             = TAC.GetConfigVariable('tasifyanticheat.webhook', 'string'),
        BypassEnabled       = TAC.GetConfigVariable('tasifyanticheat.bypassenabled', 'boolean'),
        VPNCheck            = TAC.GetConfigVariable('tasifyanticheat.VPNCheck', 'boolean', true),
        VPNKey              = TAC.GetConfigVariable('tasifyanticheat.VPNKey', 'string')
    }
...
TAC.LogBanToDiscord = function (data)
    ...
    local discordInfo = {
        ...
        ["footer"] = {
            ["text"] = 'tasifyanticheat | ' .. TAC.Version
        }
    }

    PerformHttpRequest(TAC.Config.Webhook, function(err, text, headers) end, 'POST', json.encode({ username = 'tasifyanticheat', embeds = { discordInfo } }), { ['Content-Type'] = 'application/json' })
end
...
TAC.RegisterServerCallback('tasifyanticheat:getServerConfig', function(source, cb)
    while not TAC.ConfigLoaded do
        Citizen.Wait(10)
    end
...
TAC.RegisterServerCallback('tasifyanticheat:getRegisteredCommands', function(source, cb)
    cb(GetRegisteredCommands())
end)

TAC.RegisterServerEvent('tasifyanticheat:banPlayer', function(source, type, item)
...
TAC.RegisterServerEvent('tasifyanticheat:playerResourceStarted', function(source)
...
TAC.RegisterServerEvent('tasifyanticheat:logToConsole', function(source, message)
    print(message)
end)
